...
function Table({ data }:{data:any}) {
  let headers = data.headers.map((header:ReactNode, index:number) => (
    <th key={index}>{header}</th>
  ));
  let rows = data.rows.map((row:ReactNode[], index:number) => (
    <tr key={index}>
...
function RoundedImage({alt,src,...props}:RoundedImageProps) {
  return <Image src={src} alt={alt} className="rounded-lg" {...props} />;
}
